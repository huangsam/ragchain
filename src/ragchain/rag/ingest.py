...
async def ingest(
    ...
    # Process each fetched page in turn. We update the report counters as we go
    # so partial progress is easily available even if later pages fail.
    for p in pages:
        ...
        # If we have a summary paragraph, prepend it to the full text so that
        # the chunker treats the summary as high-priority context for search.
        ...
        # If a vector store is provided, prepare documents and upsert them.
        # We construct deterministic document IDs using a safe title and the
        # chunk index to avoid accidental collisions across runs.
        if vectorstore:
            docs = []
            title_safe = safe_filename(p.get("title", "unknown"))
            for i, (chunk, vec) in enumerate(zip(chunks, embeddings)):
                # Use a stable doc id (title + index) so tests and idempotent
                ...
                docs.append(
                    {
                        "id": doc_id,
                        "text": chunk,
                        "metadata": {"title": p.get("title", ""), "chunk_index": i, "source": "wikipedia"},
                        "embedding": vec,
                    }
                )
